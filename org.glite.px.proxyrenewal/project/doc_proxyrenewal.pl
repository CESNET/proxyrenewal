...
-v  Verbose output
	...
	check_req("rm", "mv", "mkdir", "grep", "egrep", "wget", "cat", "awk", "sed", "pdflatex", "base64", "rman", "gnuhtml2latex");

	printf("\nSetting up...");
	$TMPDIR=$ENV{'TMPDIR'};
	if ($TMPDIR eq "") {$TMPDIR="/tmp";}

#	$WRKDIR="/tmp/prdoc_2989"; # XXX: back to autodetect
	$WRKDIR="$TMPDIR/prdoc_$$";
...
#	system("rm -f $WRKDIR/*");
	...
	for $file (keys %files) {
		system("wget -O @files{$file} @urls{$file}");
	}
	...
	foreach $file (@{ $conversions{'FromText'} }) {
		readme_to_tex("@files{$file}", "@files{$file}.tex", @chapters{$file}); }
	...
	for $doc ( keys %docs ) {
		...
		$texfilename = sprintf "ProxyRenewal_$doc.tex";
		...
		foreach $chapter (@{ $docs{$doc} }) {
			...
			system("echo \"\\input{@files{$chapter}.tex}\" >> $texfilename");
		}
	}
	...
	printf("\n\nOutput is in $WRKDIR\n");
	for $doc ( keys %docs ) {
		$pdffilename = sprintf "ProxyRenewal_$doc.pdf";
...
sub man_to_tex {
	...
	$tofile = $_[1];
	$chaptername = $_[2];
	system("rman -f HTML -S $fromfile > $fromfile.html");
	system("gnuhtml2latex -n $fromfile.html $qopt_devnull");
	...
	# Custom fixes
#	system("sed -i 's/^\\\\par \\\\\\\\//' $fromfile.tex"); #Remove empty paragraphs
	open TEXFILE, "$fromfile.tex" or die "$fromfile.tex not created\n";
	while (<TEXFILE>) { $fullstring .= $_ }
	close TEXFILE;
	$fullstring =~ s/\n\\par//g; #Remove empty paragraphs
	$fullstring =~ s/\n\]/]/g; #Remove empty lines before ]
	$fullstring =~ s/\n\]/]/g; #Again, if there are any left
	$fullstring =~ s/\nTable of Contents\\par//g; #Remove reference to ToC
